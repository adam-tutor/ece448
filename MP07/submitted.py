...
def minimax(board, side, flags, depth):
    ...
    if depth == 0:
      ...
      for move in generateMoves(board, side, flags):
        newSide, newBoard, newFlags = makeMove(side, board, move[0], move[1], flags, move[2])
        tempMoveList, tempMoveTree, tempScore = minimax(newBoard, newSide, newFlags, depth - 1)
        if side:
          if tempScore < minimumScore:
            minimumMove = [move] + [m for m in tempMoveList]
            minimumScore = tempScore
        else:
          if tempScore > maxScore:
            maxMove = [move] + [m for m in tempMoveList]
            maxScore = tempScore
        moveTree[encode(*move)] = tempMoveTree
      score = minimumScore if side else maxScore
      moveList = minimumMove if side else maxMove
    return (moveList, moveTree, score)
...
def alphabeta(board, side, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if depth == 0:
      ...
      for move in generateMoves(board, side, flags):
        newSide, newBoard, newFlags = makeMove(side, board, move[0], move[1], flags, move[2])
        tempMoveList, tempMoveTree, tempScore = alphabeta(newBoard, newSide, newFlags, depth - 1, alpha, beta)
        if side:
          if tempScore < minimumScore:
            minimumMove = [move] + [m for m in tempMoveList]
            minimumScore = tempScore
            beta = tempScore if tempScore < beta else beta
        else:
          if tempScore > maxScore:
            maxMove = [move] + [m for m in tempMoveList]
            maxScore = tempScore
            alpha = tempScore if tempScore > alpha else alpha
        moveTree[encode(*move)] = tempMoveTree
        if alpha >= beta:
          score = minimumScore if side else maxScore
          moveList = minimumMove if side else maxMove
