...
def get_returns(rollout_buffer: utils.RolloutBuffer, discount_factor=0.95):
    ...
    """
    # YOUR CODE HERE
    #Initialize rewards for terminal states
    returns = []
    terminal_reward = 0
    
    for i in range(len(rollout_buffer.rewards) - 1, -1, -1):
        if rollout_buffer.terminateds[i]:
            terminal_reward = rollout_buffer.rewards[i]
        else:
            terminal_reward = rollout_buffer.rewards[i] + discount_factor * terminal_reward
        returns.insert(0, terminal_reward)
...
def get_advantages(value_net: nn.Module,
    ...
    advantage_std = (advantage - torch.mean(advantage))/(torch.std(advantage) + EPSILON) 
    return advantage_std
...
def get_value_net_loss(value_net: nn.Module,
    ...
    """
    # YOUR CODE HERE
    return torch.nn.functional.mse_loss(value_net(observation), returns) 
...
def get_vanilla_policy_gradient_loss(policy: nn.Module,
    ...
    """
    # YOUR CODE HERE
    vanilla_gradient_loss = torch.tensor(0.0)
    policy_tensor = policy(observation)
    for batch, i in enumerate(action):
        vanilla_gradient_loss += (policy_tensor[batch, i[0]] * return_or_advantage[batch, 0])
    vanilla_gradient_loss /= -len(action)
    
    return vanilla_gradient_loss
...
def collect_rollouts(env: utils.EnvInterface,
    ...
    for i in range(num_rollouts):
        ...
        while not terminated:
            ...
            next_obs, terminated, reward = env.step(action) #returns observation, terminated, reward, so observation should be the next one we step to
    ...
    while r < rollouts:
        ...
        for _ in range(training_epochs_per_rollout):
            ...
            for batch_start in range(0, rollout_buffer.final_size, minibatch_size):
                ...
                index = idxr_base[batch_start:batch_stop]

                policy_gradient_kwargs = dict(
                    policy=                 policy,
                    value_net=              value_net,
                    critic=                 None,
                    observation=            rollout_buffer.observations[index],
                    old_logits=             rollout_buffer.old_logits[index],
                    action=                 rollout_buffer.actions[index],
                    return_or_advantage=    returns[index] if get_advantages == None else advantages[index],
...
def get_PPO_policy_gradient_loss (policy: nn.Module,
    ...
    action_probs_old = torch.gather(torch.log_softmax(old_logits, dim=-1), -1, action)
    action_probs_new = torch.gather(torch.log_softmax(new_logits, dim=-1), -1, action)
    prob_ratio = torch.exp(action_probs_new - action_probs_old + EPSILON)
    clipped_prob_ratio = torch.clamp(prob_ratio, 1 - ppo_clip, 1 + ppo_clip)
    policy_loss = torch.mean(-torch.min(prob_ratio * return_or_advantage, clipped_prob_ratio * return_or_advantage))
