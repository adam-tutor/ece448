...
def utility_partials(R, x):
    ...
    sig_x0 = sig2(x[0])
    sig_x1 = sig2(x[1])
    dsig_x0 = dsig2(sig_x0)
    dsig_x1 = dsig2(sig_x1)
    partial = np.array([dsig_x0 @ R[0] @ sig_x1, sig_x0 @ R[1] @ dsig_x1])
    return partial
...
def episodic_game_gradient_ascent(init, rewards, nsteps, learningrate):
    ...
    logits[0] = init
    for i in range(2):
        utilities[0,i] = sig2(init[i]) @ rewards[i] @ sig2(init[1])
    for t in range(1, nsteps):
        logits[t] = logits[t-1] + learningrate * utility_partials(rewards, logits[t-1]) 
        for i in range(2):
...
def episodic_game_corrected_ascent(init, rewards, nsteps, learningrate):
    ...
    for i in range(2):
        utilities[0,i] = sig2(init[i]) @ rewards[i] @ sig2(init[1])
    for t in range(1, nsteps):
        partials = utility_partials(rewards, logits[t-1])
        hessian = utility_hessian(rewards, logits[t-1])
        correction = symplectic_correction(partials, hessian)
        logits[t] = logits[t-1] + learningrate * (np.eye(2) + correction) @ partials 
        for i in range(2):
            utilities[t, i] = sig2(logits[t,0]) @ rewards[i] @ sig2(logits[t,1]) 
    return logits, utilities
